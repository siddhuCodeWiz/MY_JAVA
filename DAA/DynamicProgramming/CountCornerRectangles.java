...
public class CountCornerRectangles {
    ...
    public static void main(String[] args) {
        ...
        CountCornerRectangles ccr = new CountCornerRectangles();
        ccr.m = sc.nextInt();
        ccr.n = sc.nextInt();

        ccr.arr = new int[ccr.m][ccr.n];

        for(int i=0; i<ccr.m; i++){
            for(int j=0; j<ccr.n; j++){
                ccr.arr[i][j] = sc.nextInt();
            }
        }
    }


    public void calculateRecs(){
